...
func main() {
	...
	fmt.Printf("Servidor escuchando en: http://localhost:8002/") //	message (using fmt package)
	http.ListenAndServe(":8002", nil)                            //	listens on the TCP network address (localhost:8000) and then calls Serve to handle requests on incoming connections.
}
